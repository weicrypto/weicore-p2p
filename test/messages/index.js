...
describe('Messages', function() {
  ...
  describe('@constructor', function() {
    it('sets properties correctly', function() {
      var network = weicore.Networks.defaultNetwork;
      var messages = new Messages({
        network: network,
        Block: weicore.Block,
      ...
      messages.builder.constructors.Block.should.equal(weicore.Block);
      messages.builder.constructors.Transaction.should.equal(weicore.Transaction);
    ...
    it('network should be unique for each set of messages', function() {
      ...
      messages.network.should.deep.equal(weicore.Networks.livenet);
      messages2.network.should.deep.equal(weicore.Networks.testnet);
      var message1 = messages.Version();
      message1.network.should.deep.equal(weicore.Networks.livenet);
      var message2 = messages2.Version();
      message2.network.should.deep.equal(weicore.Networks.testnet);
    });
  });
  ...
  describe('Default Network', function() {
    ...
    Object.keys(messages.builder.commandsMap).forEach(function(command) {
      ...
      it(name, function() {
        var message = messages[name]();
        message.network.should.deep.equal(weicore.Networks.defaultNetwork);
      });
    });
